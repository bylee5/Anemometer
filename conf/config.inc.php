 ...
 **/
$conf['default_report_action'] = 'graph_search';
...
$conf['history_defaults'] = array(
	...
	'fact-limit' => '60',
	'dimension-ts_min_start' => date("Y-m-d H:i:s", strtotime( '-60 day')),
	...
	#'table_fields' => array('date', 'index_ratio','query_time_avg','rows_sent_avg','ts_cnt','Query_time_sum','Lock_time_sum','Rows_sent_sum','Rows_examined_sum','hostname_max','db_max','user_max','ip_max')
	'table_fields' => array('date', 'index_ratio','query_time_avg','rows_sent_avg','ts_cnt','Query_time_sum','Query_time_max','Lock_time_sum','Rows_sent_sum','Rows_examined_sum','Rows_examined_max','hostname_max','db_max','user_max','ip_max')
);
...
$conf['report_defaults'] = array(
	...
	'table_fields' => array('checksum','snippet', 'index_ratio','query_time_avg','rows_sent_avg','ts_cnt','Query_time_sum','Lock_time_sum','Rows_sent_sum','Rows_examined_sum','hostname_max','db_max','user_max','ip_max'),
...
$conf['plugins'] = array(
	...
	'visual_explain' => '/bin/pt-visual-explain',
...
#	'query_advisor'	=> '/usr/bin/pt-query-advisor',
	...
	'explain'	=>	function ($sample) {
		...
		if ($conn['host'] == 'db1.tmonc.net' || $conn['host'] == 'db2.tmonc.net' )
		{
			$conn['host'] = 'checkdb.tmonc.net';
		}elseif ($conn['host'] == '부킹') {
			$conn['host'] = '127.0.0.1';
			$conn['port'] = 3306;
		}
...
$conf['reports']['slow_query_log'] = array(
	// joins
	'join'	=> array (
		'dimension'    => 'USE INDEX (idx_ts_min_01) USING (`checksum`)'
	),
	...
	'custom_fields'	=> array(
		...
		'snippet' => 'LEFT(dimension.sample,100)',
		...
		'rows_examined_avg' => 'ROUND(SUM(Rows_examined_sum)/SUM(ts_cnt),0)',
		'rows_examined/sent_avg' => 'ROUND(SUM(Rows_examined_sum)/SUM(Rows_sent_sum)/SUM(ts_cnt),0)',
	),

	'callbacks'     => array(
		'table' => array(
			'date'  => function ($x) { $type=''; if ( date('N',strtotime($x)) >= 6) { $type = 'weekend'; } return array($x,$type); },
			'checksum' => function ($x) { return array($x); }
		)
	)
